...
from extractionProcess import extract_full_data
from csvHandler import initialize_csv, append_to_csv
...
# Run this: 
# "C:\Program Files\Google\Chrome\Application\chrome.exe" --remote-debugging-port=0 --user-data-dir="C:\Users\DELL\AppData\Local\Google\Chrome\User Data" 
...
async def process_row(driver, street, zipcode, target_name_set, fallback_street_data, fallback_zip_data, processed_queries, i):
    try:
        ...
        extracted_data = await extract_full_data(driver, target_name_set)
        await random_delay(1, 5)  # Add random delay
        ...
        phone_numbers = extracted_data.get('phoneNumbers', [])
        phone_types = extracted_data.get('phoneTypes', [])
        email_data = extracted_data.get('emailData', [])
        ...
        data_to_write = {
            "Mailing Address": extracted_data.get('currentMailingAddress', ''),
            ...
            "First Name": extracted_data.get('firstName', ''),
            "Last Name": extracted_data.get('lastName', ''),
        ...
        append_to_csv(data_to_write, 'PA Data - Cape Coral Dry Lots.csv')
...
async def main():
    ...
    try:
        ...
        street_data = get_sheet_data(SHEET_ID, "PA Data - Cape Coral Dry Lots!D2:D")
        zip_data = get_sheet_data(SHEET_ID, "PA Data - Cape Coral Dry Lots!D2:D")
        target_names = [
            split_target_names(get_sheet_data(SHEET_ID, f"PA Data - Cape Coral Dry Lots!{col}2:{col}"))
            for col in ["C", "R"]
        ]
        fallback_street_data = [
            get_sheet_data(SHEET_ID, f"PA Data - Cape Coral Dry Lots!{col}2:{col}")
            for col in ["B"]
        ]
        fallback_zip_data = [
            get_sheet_data(SHEET_ID, f"PA Data - Cape Coral Dry Lots!{col}2:{col}")
        ...
        officer_details_data = get_sheet_data(SHEET_ID, "PA Data - Cape Coral Dry Lots!R2:R")
        ...
        for i, street_entry in enumerate(street_data):
            ...
            if not street or not zipcode or not target_name_set:
                print(f"Skipping row {i + 2}: Invalid data.")
                continue
